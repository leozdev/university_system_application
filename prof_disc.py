...
def incluir(database1, database2, database3):
    ...
    # Cruzar dados:
    # registro na database2
    # sigla na database 3
    # ano, semestre, dias, horarios, curso tudo novo

    registro = input('Registro Funcional: ')

    if registro in database2.keys():

        sigla_disciplina = input('Sigla da Disciplina: ')

        if sigla_disciplina in database3.keys():

            ano, semestre, dias_da_semana, horarios_inicio, curso = dados()

            if registro not in database1.keys():
                database1[registro] = {}
...
def listar_elemento_especifico(database, chave):
    ...
